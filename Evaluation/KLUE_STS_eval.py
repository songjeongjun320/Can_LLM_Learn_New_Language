...
# 모델 설정들 (기본 OLMo 1B, OLMo 7B, Llama3.2)
MODEL_CONFIGS = [
    ModelConfig(
        name="OLMo-1b", 
...
def load_model_and_tokenizer(model_config):
    ...
    if model_config.is_ollama:
        # Ollama 서버를 통한 모델 사용 (Llama3.2)
        logger.info(f"Using Ollama server at {model_config.ollama_host} for {model_config.name}")
        # 토크나이저만 로컬에서 로드 (Llama3는 HuggingFace 토크나이저 사용)
        tokenizer = AutoTokenizer.from_pretrained("meta-llama/Meta-Llama-3-8B")
        
        # Ollama 클라이언트 생성 (실제 모델은 추론 시 사용됨)
        ollama_client = ollama.Client(host=model_config.ollama_host)
        
        # HuggingFace와 호환되는 방식으로 Ollama 모델 사용을 위해 래퍼 모델 로드
        # 실제 학습 시에는 HuggingFace 모델로 로컬 파인튜닝
        model = AutoModelForCausalLM.from_pretrained(
            "meta-llama/Meta-Llama-3-8B",  # 로컬 파인튜닝을 위한 모델 구조 참조
            torch_dtype=torch.bfloat16,
            device_map="auto"
        )
        
        # 모델 객체에 Ollama 클라이언트 참조 추가 (나중에 평가에서 사용)
        model.ollama_client = ollama_client
        model.ollama_model_name = model_config.model_path
        ...
        # OLMo 모델에 필요한 trust_remote_code 설정
        trust_remote = "OLMo" in model_config.model_path
        ...
        model = AutoModelForCausalLM.from_pretrained(
            ...
            trust_remote_code=trust_remote  # OLMo 모델에 필요
        )
...
def evaluate_model(model, tokenizer, model_config):
    ...
    for item in tqdm(val_subset):
        ...
        # Ollama API 사용 또는 일반 모델 사용
        if model_config.is_ollama:
            # Ollama API를 통한 추론
            try:
                response = model.ollama_client.generate(
        ...
        log_data = {
            ...
            "predicted_score": predicted_score if match else None  # 예측 점수
        }
    ...
    if true_scores and pred_scores:
        ...
        # NLP 평가 지표 (BLEU와 ROUGE 예시)
        # BLEU score는 일반적으로 기계 번역에서 많이 사용되지만, 다른 NLP 작업에도 사용할 수 있음
