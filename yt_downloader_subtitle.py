...
def download_subtitles(url, video_name, output_folder_video='yt_dataset/videos', output_folder_subtitle='yt_dataset/subtitles'):
    # 출력 폴더가 없으면 생성
    ...
    ydl_opts = {
        ...
        'outtmpl': os.path.join(output_folder_subtitle, f'{video_name}.%(ext)s'),  # 자막 파일 경로
        'quiet': True,  # 로그 출력 방지
        'skip_download': True,  # 비디오나 오디오를 다운로드하지 않음
    }
    ...
    # 자막 파일 경로
    subtitle_filename = os.path.join(output_folder_subtitle, f'{video_name}.ko.vtt')
...
def main(url, video_name):
    # 영상 및 자막 다운로드
    subtitle_filename = download_subtitles(url, video_name)
    print("영상 및 자막 다운로드 완료")
    print("자막 파일: ", subtitle_filename)
